...
import { getProjectThumbnail, projects, type Project } from "../portfolio-data";
    ...
    <dialog className="project-dialog" ref={dialog} aria-labelledby="project-dialog-title" aria-describedby="project-dialog-description"
      ...
      <div className="project-dialog-shell">
        <header className="project-dialog-header">
          <div className="project-author"><span className="project-author-avatar" aria-hidden="true"><img src="/optimized/grethiel-avatar-128.webp" alt="" width="128" height="192" /></span><div><strong>Grethiel Joy</strong><span>Digital Marketing & Web Development</span></div></div>
        ...
        <div className="project-dialog-body">
          ...
          <section className="project-creator-section" aria-labelledby="project-creator-title">
            <div className="project-creator-rule" aria-hidden="true"><span><img src="/optimized/grethiel-avatar-128.webp" alt="" width="128" height="192" loading="lazy" decoding="async" /></span></div>
            ...
            <div className="related-project-grid">
              {relatedProjects.map(item => {
                ...
                <button className="related-project-card" type="button" key={item.slug} onClick={() => onSelect(item.slug)} aria-label={`View ${item.title} project`}>
                  <span className="related-project-image"><img src={thumbnail.src} srcSet={thumbnail.srcSet} sizes="(max-width: 520px) 46vw, 22vw" alt="" width="800" height="600" loading="lazy" decoding="async" /></span>
            ...
            <div className="service-offerings-grid">
              {services.map(service => (
                <a className="service-offering-card" href="/contact" key={service.title}>
                  <span className="service-offering-image"><img src={service.image} srcSet={`${service.image.replace("/projects/thumbnails/", "/projects/thumbnails/mobile/")} 400w, ${service.image} 800w`} sizes="(max-width: 520px) 46vw, 22vw" alt="" width="800" height="600" loading="lazy" decoding="async" /></span>
          ...
          <article className="gallery-card" key={project.slug}>
            <button className="gallery-card-open" type="button" onClick={() => setProjectHash(project.slug)} aria-label={`View ${project.title} project`} aria-haspopup="dialog">
              <span className="gallery-image-frame">
                <img src={thumbnail.src} srcSet={thumbnail.srcSet} sizes="(max-width: 520px) calc(100vw - 2rem), (max-width: 900px) 88vw, (max-width: 1100px) 46vw, 31vw" alt={project.alt} width={800} height={600} loading={projectIndex === 0 ? "eager" : "lazy"} fetchPriority={projectIndex === 0 ? "high" : "auto"} decoding="async" />
